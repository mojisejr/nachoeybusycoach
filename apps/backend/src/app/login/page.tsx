...
export default function LoginPageWrapper() {
  // Development logging to track wrapper mounting
  if (process.env.NODE_ENV === 'development') {
    console.log('[LoginPageWrapper] Component mounted with SessionProvider');
  }
...
function LoginPage() {
  ...
  useEffect(() => {
    // Development logging to track component mounting
    if (process.env.NODE_ENV === 'development') {
      console.log('[LoginPage] Component mounted, checking session...');
    }
    
    // Check if user is already authenticated
    getSession().then((session: Session | null) => {
      if (process.env.NODE_ENV === 'development') {
        console.log('[LoginPage] Session check result:', session ? 'authenticated' : 'not authenticated');
      }
      if (session) {
      ...
      <div className="max-w-md w-full space-y-8">
        <div>
          <h2 className="mt-6 text-center text-3xl font-extrabold text-gray-900">
            เข้าสู่ระบบ NachoeyBusyCoach
          </h2>
          <p className="mt-2 text-center text-sm text-gray-600">
            แพลตฟอร์มการสื่อสารระหว่างนักวิ่งและโค้ช
          </p>
        </div>
